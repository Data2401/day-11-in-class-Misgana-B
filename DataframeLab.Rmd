...
```{r}
sum(grants$start_year == 2010, na.rm = TRUE)
...
```{r}
sum(grants$total_amount[grants$start_year == 2010], na.rm = TRUE)
```
...
expenditure_df["Personal Care", "1940"]
```
...
```{r}
expenditure_df["Food and Tobacco", "1960"]
...
```{r}
expenditure_df$Category[which.max(expenditure_df$`1960`)]
...
```{r}
expenditure_df$Category[which.min(expenditure_df$X1960)]
...
lowest_category <- function(year) {
  ...
  expenditure_df$Category[which.min(expenditure_df[[col_name]])]
}
```
...
```{r}
years <- gsub("X", "", colnames(expenditure_df)[1:5]) 
